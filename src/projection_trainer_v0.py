...
class VJEPAProjection(nn.Module):
    ...
    Two-layer MLP with residual-style normalization.
    """
...
class PanelSequenceDataset(Dataset):
    """
    Dataset of (vjepa_sequence_embedding, clip_text_embedding) pairs.
    Uses CLIP panel action labels as pseudo-labels.
    """
    def __init__(
        self,
        ...
        for meta in seq_metadata:
            # Use majority action label from panels in sequence
            action_votes = {}
            ...
            if action_votes:
                ...
                action_idx      = action_to_idx.get(majority_action, 4)  # default: angry
            else:
                action_idx = 4  # default: angry
...
def load_sequence_data(manga_name: str):
    """Load saved V-JEPA 2 sequence embeddings and metadata."""
    seq_emb_path  = OUTPUT_DIR / f'{manga_name}_vjepa_seq_embeddings.npy'
    seq_meta_path = OUTPUT_DIR / f'{manga_name}_vjepa_seq_metadata.json'

    if not seq_emb_path.exists():
        print(f'Sequence embeddings not found for {manga_name}')
        print('Saving them now from action labels...')
        return None, None
...
def train_projection(
    ...
    print(f'\nTraining projection layer: {sum(p.numel() for p in model.parameters())/1e6:.2f}M params')
    ...
    for idx, count in label_counts.items():
        class_weights[idx] = total_samples / (len(label_counts) * count)
    ...
    for epoch in range(epochs):
        ...
        for vjepa_emb, target_text_emb, action_idx in dataloader:
            ...
            weights    = class_weights[action_idx]
            cos_loss   = (weights * (1 - cos_sims)).mean()
...
def evaluate_projection(device: str = 'cuda'):
    """
    Evaluate trained projection on all volumes.
    Compare agreement with CLIP labels before and after projection.
    """
    if not PROJECTION_PATH.exists():
        print('No trained projection found. Run training first.')
        return
    ...
    for vol in VOLUMES:
        ...
        print(f'  Projected distribution:')
    ...
    print(f'Improvement: {overall - 0.017:+.3f}')
...
if __name__ == '__main__':
    ...
    # Check sequence embeddings exist
    missing = []
    for vol in VOLUMES:
    ...
    if missing:
        print(f'\nMissing sequence embeddings for: {missing}')
        print('Need to save them from sequential_encoder.py first.')
        print('Add this to sequential_encoder.py run_vjepa_action_detection():')
        print('  np.save(OUTPUT_DIR / f"{manga_name}_vjepa_seq_embeddings.npy", seq_embeddings)')
        print('  with open(OUTPUT_DIR / f"{manga_name}_vjepa_seq_metadata.json", "w") as f:')
        print('      json.dump(seq_metadata, f)')
    else:
